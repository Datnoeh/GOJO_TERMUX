...
echo -e "\e[1;35mQuá Trình Setup Đang Được Bắt Đầu Vui Lòng Không Thoát Termux .........";
 yes | (pkg update && pkg upgrade && pkg install wget && pkg install openjdk-17 && pkg install ant && pkg install dropbear && pkg install openssh && pkg install tsu && pkg install net-tools && pkg install python && pkg install php && pkg install openssl-tool && pkg install zip) > /dev/null 2>&1
